...
var (
	...
	ErrEnumRoleEnumValidation = govaliderrors.ValidationError{Reason: "field EnumRole must be one of [admin, user, guest]", Path: "Enum.Role"}

	// ErrEnumLevelEnumValidation is the error returned when the value is not in the allowed enum values [1, 2, 3].
	ErrEnumLevelEnumValidation = govaliderrors.ValidationError{Reason: "field EnumLevel must be one of [1, 2, 3]", Path: "Enum.Level"}
	...
	ErrEnumUserRoleEnumValidation = govaliderrors.ValidationError{Reason: "field EnumUserRole must be one of [manager, developer, tester]", Path: "Enum.UserRole"}

	// ErrEnumPriorityEnumValidation is the error returned when the value is not in the allowed enum values [10, 20, 30].
	ErrEnumPriorityEnumValidation = govaliderrors.ValidationError{Reason: "field EnumPriority must be one of [10, 20, 30]", Path: "Enum.Priority"}
)
